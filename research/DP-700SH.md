...
### ITE `IT9834` / `IT9836` は2010年DPシリーズで優先照合すべきSoC候補

2009-12-10、台湾のDigiTimesは聯陽半導體（ITE Tech）が「デジタルフォトフレーム専用」の3Dグラフィックス加速SoC `IT9834` / `IT9836` を正式発表したと報じた。

記事による確定事項:
- `IT9834` / `IT9836` はデジタルフォトフレーム専用SoCとして発表された。
- 発表時点で「國外大廠指定使用」（海外大手メーカーから指定採用）を得たとされる。
...
- ITEは複数の顧客・パートナーとこの系列を使った完成品を開発し、2010年第1四半期に順次発表するとしていた。
- このSoCは旧SMedia（繪展科技）のグラフィックス技術を継承した製品と説明されている。
...
**状況証拠:** 既知の台湾側 `FUJIFILM DP-850SH/DP-1020SH / Customer: Sharp` 開発期間は2009-10〜2010-02で、ITEの発表はその真ん中に位置する。ITEが顧客完成品の投入時期として挙げた2010Q1もDP-700SH / DP-850SH / DP-1020SHの市場投入時期と重なる。さらに媒体・JPEG/動画/音声機能の統合内容もDPシリーズ、とくに850SH/1020SHの機能構成と整合する。このため `IT9834` / `IT9836` を基板写真・ファームウェア取得時の優先照合候補に追加する。

**断定禁止:** DP-700SH / DP-850SH / DP-1020SHへの実搭載は未確認。DigiTimesがいう「海外大手」がSharp/FUJIFILMだった証拠もない。Dusty Shyr氏とITE/SMediaの雇用関係も未確認である。2021年以降のITE公開資料にも同系列型番があるが、そこに記載されたRISC-V等の現代的仕様を2009年発表品へ遡及してはならない。

### SanJetは2009年6月時点で「デジタルフォトフレーム」を商標の指定商品として出願

台湾の経済部智慧財産局公開資料を転載する商標記録で、勝捷光電股份有限公司（SanJet）が2009-06-26に出願した商標 `勝捷`（申請案号 `098027401`、登録番号 `01391873`）を確認した。登録日は2010-01-01。

指定商品（第009類）には、**`數位相框`（デジタルフォトフレーム）** が明記されている。ほかにデジタルカメラ、デジタル写真プレーヤー、プロジェクター、デジタルビデオカメラ、MP3プレーヤー、携帯メディアプレーヤーなど画像・携帯AV機器が並ぶ。

参考:
- https://www.findcompany.com.tw/trademark/01391873_098027401

**確定:** SanJetはAIPTEKのODM事業承継日（2009-06-01）の約3週間後には、少なくとも自社商標の事業対象としてデジタルフォトフレームを明示していた。登録日の2010-01-01は、既知の台湾側 `FUJIFILM DP-850SH/DP-1020SH / Customer: Sharp` 開発期間（2009-10〜2010-02）の途中に当たる。

**状況証拠:** AIPTEKのODM事業承継、AIPTEKのDPF特許のSanJetへの移転、同一担当者のAIPTEK案件→Sharp向けFUJIFILM DPF案件という連続性に加え、SanJet自身がまさにその時期にDPFを事業対象としていたことが追加された。SanJetを台湾側ODM/開発母体の候補とする仮説を補強する。

**断定禁止:** 商標の指定商品は実際の受注製品や顧客名を示さない。したがって、この商標だけからSanJetがDP-700SH / DP-850SH / DP-1020SHを設計・製造したとは断定しない。

### AIPTEKの光センサー式表示制御特許もSanJetへ移転

中国特許 **CN1260698C**（自動調光システム）は、AIPTEK International / 天瀚科技を元の権利者とし、後にSanJet Optoelectronicsへ移転している。特許本文は静止画像表示装置の例としてデジタルフォトフレームを明示し、周囲光を検出する光センサー、制御回路、表示部・バックライト等からなる自動調光系を記述する。

参考:
- https://patents.google.com/patent/CN1260698C/zh

**確定:** AIPTEKからSanJetへ移った技術資産には、外枠構造だけでなく、デジタルフォトフレームに適用可能な光センサー式表示制御技術も含まれていた。

**状況証拠:** DP-700SH / DP-850SH / DP-1020SHはいずれも背面の明るさセンサーを利用した省電力制御を持つため、SanJet側が当時DPFの光センサー制御に関する設計資産を継承していた事実は技術領域として整合する。

**断定禁止:** CN1260698Cの制御は自動調光を中心とする一方、DPシリーズで確認されているのは暗所での自動電源OFFを含む省電力機能である。回路・ソフトウェア・アルゴリズムがDPシリーズに流用された証拠はなく、特許との一致を実装証拠として扱わない。

### 2010年3機種の付属リモコンは外観・電池仕様を共有する可能性が高い

2026-09-10、DP-850SHおよびDP-1020SHの中古一式写真を再調査し、各機種に付属するFUJIFILMリモコンが、DP-700SHで型番を直接確認済みの `RRMCG2009SCZZ` と同じ筐体・同じボタン配置に見えることを確認した。さらに別々の中古流通記録で、DP-850SH / DP-1020SHとも付属リモコン用電池として **CR2025** が明記される。DP-700SHの `RRMCG2009SCZZ` も背面ラベルでCR2025×1を直接確認済み。

参考:
- DP-850SH 一式写真: https://jp.mercari.com/item/m32657287888
- DP-850SH CR2025記載: https://fril.jp/brand/4966/category/1349
- DP-1020SH 一式写真: https://auctions.yahoo.co.jp/jp/auction/j1048872798
- DP-1020SH CR2025記載: https://jp.mercari.com/item/m34411055966
- DP-700SH `RRMCG2009SCZZ`: https://auctions.yahoo.co.jp/jp/auction/q1171811726

**確定度: 中:** 2010年3機種の付属リモコンは、確認できる範囲で筐体・ボタン配置・電池規格が一致する。

**状況証拠:** 3機種でリモコンハードウェアとコマンド体系が共通化されていた可能性が高まった。DP-850SH / DP-1020SH付属リモコンも、生IR波形を採取するためのドナー候補として優先的に確認する価値がある。

**断定禁止:** DP-850SH / DP-1020SH側は背面ラベルの型番をまだ直接読めていない。両機種にも正式に `RRMCG2009SCZZ` が付属したこと、赤外線プロトコル・アドレス・各キーコードがDP-700SHと完全一致することは未確定。

### SanJetは台湾R&Dと中国製造を一体で扱う体制を持っていた

2014-10-24のDigiTimes記事は、勝捷光電（SanJet）を **「2009年6月1日正式成立」** とし、研究開発・運営拠点を新竹科学園区、中国・蘇州呉江に製造拠点を置く会社として説明している。2013-12-27にiThomeが掲載したIntel Capitalの発表でも、SanJet側は設計から生産までを覆う垂直統合能力、成熟したエンジニアリングチーム、多数の一線メーカー顧客を持つと説明している。

参考:
- https://www.digitimes.com.tw/tech/dt/n/shwnws.asp?id=0000398830_UIF9UDYF2AYO4W1AFVXO2
- https://www.ithome.com.tw/pr/84531

**確定事項の整理:** 台湾の会社登記で確認済みの法的な設立日は `2008-11-20`。一方、複数の後年資料がいう「2009-06-01正式成立」は、既知のAIPTEK ODM事業の分割・承継日と一致する。したがって両者を矛盾として扱わず、2008-11-20を法人登記上の設立、2009-06-01を事業上の正式発足・承継効力日として区別するのが安全である。

**状況証拠:** 既知の `AIPTEK ODM事業 → SanJet` の承継、SanJetのDPF商標・DPF関連知財、台湾側技術者の `FUJIFILM DP-850SH/DP-1020SH / Customer: Sharp` 案件に加え、SanJetが台湾側R&Dと中国側製造をつなぐ一貫体制を持っていたことが確認できた。DP-700SH / DP-850SH / DP-1020SHの実機銘板がすべて `製造元: シャープ株式会社` / `MADE IN CHINA` である事実とは整合するため、SanJetを台湾側ODM/開発母体の候補とする仮説を補強する。

**断定禁止:** DigiTimes/iThome資料はSharpまたはFUJIFILMをSanJetの顧客として挙げていない。DPシリーズが蘇州呉江工場で組み立てられた証拠もなく、2014年の記事に記載された製造拠点が2009-10〜2010-02のDPシリーズ開発時点ですでに稼働していたことも直接確認できない。Sharp↔SanJetの契約・発注・基板識別情報が得られるまではODMを確定しない。

### DP-700SHは先代DP-70SHの明示的な系列継承機 — 隠し更新操作の検証価値が上昇

2010-02-02のデジカメ WatchによるDP-700SH発表記事は、同機を **「従来機種『DP-70SH』の流れを汲む7型デジタルフォトフレーム」** と明記している。単に画面サイズや外観が似ているという比較ではなく、当時の記事上で製品系列の継承関係が明示されている。

一方、2009-04-01のDP-70SHファームウェア更新記事では、Ver.1.04.00への更新手順として、`本体情報の表示` → `バージョン表示` の画面で純正リモコンを **`拡大` → `縮小` → `時計` → `回転`** の順に押すと「ファームウェア書き換え画面」に移ることが具体的に記録されている。

参考:
- DP-700SH発表記事: https://dc.watch.impress.co.jp/docs/news/346441.html
- DP-70SH更新記事: https://dc.watch.impress.co.jp/cda/accessories/2009/04/01/10602.html

**確定:** DP-70SHには上記4キー列による隠し更新画面が存在した。DP-700SHは当時の記事でDP-70SHの「流れを汲む」機種と位置づけられている。DP-700SH純正 `RRMCG2009SCZZ` には4キーすべてが存在し、手元実機にも `本体情報の表示` / `バージョン表示` がある。

**状況証拠 / 実機検証候補:** DP-700SHでも同じ入口を継承している可能性は、単なるUI類似から推す場合より高くなった。手元個体のVer.1.03.00で `拡大 → 縮小 → 時計 → 回転` を試し、更新画面へ遷移するか確認する価値が高い。遷移すれば `TH34_dpf.pkg` を読む更新ローダの存在を実機側から直接確認できる。

**断定禁止:** 「流れを汲む」はSoC、OS、ブートローダ、更新パッケージ形式が同一であることを意味しない。DP-700SHで同じ4キー列が実際に有効だという直接証拠はまだなく、DP-70SHのファームウェアをDP-700SHへ投入してはならない。

### 天擎積體（Mars Semiconductor）を追加SoC候補として記録

台湾の天擎積體電路（Mars Semiconductor）について、旧い企業紹介文を保存している台湾企業ディレクトリに、同社が **デジタルカメラ・デジタルフォトフレーム・デジタル画像処理向けSoCを開発・販売し、顧客例として `天瀚`（AIPTEK）を挙げる** 記述が残っている。天擎の現行公式沿革でも法人設立は2001年、2007年に無線事業部設置と確認できる。さらに2008年の台湾技術者の就職記録には、天擎が当時 `wireless camera` と `數位相框` の製品を開発していたとの記録がある。
...
**確定度: 中:** 天擎が2008年前後にデジタルフォトフレーム向けSoC/製品開発を行っていたこと、企業紹介上の顧客例にAIPTEK/天瀚が含まれていたことは確認できる。ただし顧客記載は企業ディレクトリに残る旧紹介文であり、現在の天擎公式サイトには同じ顧客一覧は掲載されていない。
...
`AliasFakename/ir-remotes` の `RRMCG0041SJSA.lircd.conf` は、2002-02-22にLIRCで生成された設定で、ヘッダに `brand: Sharp`、`model no. of remote control: RRMCG0041SJSA`、対象機器 `Audio System XL-3500 H` と明記される。
...
- `min_repeat 2`
- `toggle_mask 0x3ff`
- 例: `KEY_POWER 0x11A2`, `KEY_CD 0x1362`, `KEY_VIDEO 0x12E2`

このLIRCファイル自体にはcarrier frequencyの指定がないため、`RRMCG0041SJSA` の搬送波周波数はこの資料単独では確定しない。

一方、`Flipper-IRDB` のSharp `RRMCG A375 WJSA` は各キーをraw timingで保存しており、`frequency: 38000`、`duty_cycle: 0.33` が明記される。Power波形は `3208 1604 394 394 ...` から始まる長いraw列で、0041のLIRC記述とは見かけ上かなり異なる。
...
**確定:** `RRMCG...` はSharpの実リモコン型番系列として使われ、その一部には公開された実測IRデータが存在する。`RRMCG0041SJSA` は15-bitの `SPACE_ENC` として記録され、別の `RRMCG A375 WJSA` は38kHzのraw波形として記録されている。

**状況証拠 / 実装上の示唆:** DP-700SHでSharp系プロトコルを優先して総当たりする方針は、完成品製造元がSharpという事実に加え、同じ型番系列にSharp系として記録された実測例が存在する点から補強される。一方で、`RRMCG`という型番系列だけから特定のbit長・波形形式へ一意に絞るのは危険である。

**13-bit / 15-bit表記への注意:** 現在DP-700SH側では「Sharp 13-bit」として探索しているが、別機種LIRCの `bits 15` を直ちに矛盾とはみなさない。Sharp系プロトコルでは、実装やライブラリによりアドレス+コマンド部分と付加ビットをどこまでbit数に含めるかが異なる場合がある。DP-700SHを15-bitへ再分類する直接証拠ではなく、現在の13-bit生成実装が出す完全な波形を比較する材料とする。
...
### `RRMCG2009SCZZ` を DP-850SH / DP-1020SH 用と明記する流通記録

2026-09-14、`RRMCG2009SCZZ` 単体の中古出品を再検索し、型番そのものとDP-850SH / DP-1020SHを直接結びつける記録を複数確認した。

メルカリ出品 `m93734607492` は商品名を `FUJIFILM デジタルフォトフレーム用リモコン RRMCG2009SCZZ` とし、説明中に **`DP-850SH DP-1020SH`** と明記している。別出品 `m53512993273` でも `RRMCG2009SCZZ` と `DP-850SH / DP-1020SH` が併記されている。

参考:
- https://jp.mercari.com/item/m93734607492
- https://jp.mercari.com/item/m53512993273

**確定:** `RRMCG2009SCZZ` の単体中古流通記録に、適用先としてDP-850SH / DP-1020SHを明示した例が複数存在する。
...
### 先代DP-70SHの公開ファームは少なくともVer.1.07.00まで存在 — `1.04.00`一致の扱いを修正

2009-09-18のデジカメ Watch記事を再確認し、富士フイルムが2009-09-15にDP-70SH向け **Ver.1.07.00** を公開していたことを確認した。更新はSDメモリーカード経由。

変更内容:
- 電源ON時、前回まで設定・実行されていたスライドショーモードで起動するよう変更。
- 本体で読み取れない画像が含まれる場合に、一部画像しか表示されない、またはスライドショーが起動しない症状を修正。

参考:
- https://dc.watch.impress.co.jp/docs/news/316738.html
- 先行するVer.1.04.00更新: https://dc.watch.impress.co.jp/cda/accessories/2009/04/01/10602.html

**確定:** DP-70SHの公開ファームは2009-03-31のVer.1.04.00で終わらず、少なくとも2009-09-15のVer.1.07.00まで進んでいた。したがって、DP-70SHのVer.1.04.00とDP-700SHの2010年公開Ver.1.04.00が同じ番号であることは、「両機種の最終版が一致した」という事実ではない。
